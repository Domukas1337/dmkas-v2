...
interface InfiniteReviewsProps {
  animeId: number;
  initialReviews: AnimeReviews[];
}

export default function InfiniteReviews({
  animeId,
  initialReviews,
}: InfiniteReviewsProps) {
  const [reviews, setReviews] = useState<AnimeReviews[]>(initialReviews);
  ...
  const loadMoreReviews = useCallback(async () => {
    ...
    try {
      ...
      const newReviews = await getReviews(animeId, page);
